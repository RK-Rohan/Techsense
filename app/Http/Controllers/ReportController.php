        ...
        if (request()->ajax()) {
            $sells = Transaction::where('transactions.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw('transactions.final_total - IFNULL(SUM(tp.amount), 0) as total_due'),
            ...
            if (request()->filled('start_date') && request()->filled('end_date')) {
                $sells->whereBetween('transactions.transaction_date', [request('start_date'), request('end_date')]);
            }

            if (request()->input('payment_status') == 'overdue') {
                $sells->whereNotNull('transactions.pay_term_number')
