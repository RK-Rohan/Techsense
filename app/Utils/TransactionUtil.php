...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'custom_field_6' => !empty($input['custom_field_6']) ? $input['custom_field_6'] : null,
    ...
    public function agentTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        $sale_type = 'agent';
        $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
        $invoice_no = !empty($input['invoice_no']) ? $input['invoice_no'] : $this->getInvoiceNumber($business_id, $input['status'], $input['location_id'], $invoice_scheme_id, $sale_type);

        $final_total = $uf_data ? $this->num_uf($input['cnf_cost']) : $input['cnf_cost'];
        ...
        $transaction = Transaction::create([
            ...
            'status' => $input['status'],
            'sub_status' => !empty($input['sub_status']) ? $input['sub_status'] : null,
            'contact_id' => $input['agent_id'] ?? null,
            'customer_group_id' => !empty($input['customer_group_id']) ? $input['customer_group_id'] : null,
            'invoice_no' => $invoice_no,
            'ref_no' => '',
            'payment_status' => 'due',
    ...
    public function createAgentTransaction($transaction, $input, $transaction_agent)
    {
        $cnf_qty = $input['products'][1]['quantity'] ?? 0;

        $agent_transaction = AgentTransaction::create([
            'sales_id'          => $transaction_agent->id,
            'supplier_id'       => $input['supplier_id'] ?? null,
            'agent_id'          => $input['agent_id'] ?? null,
            'customer_id'       => $input['contact_id'],
            'invoice_no'        => $transaction->invoice_no,
            'cnf_qty'           => $cnf_qty,
            'cnf_rate'          => $input['cnf_rate'] ?? 0,
            'cnf_amount'        => $input['cnf_cost'] ?? 0,
            'supplier_amount'   => $input['supplier_amount'] ?? 0,
            'number_of_cartons' => $input['no_of_carton'] ?? 0,
    ...
    public function createTermsConditionLine($transaction, $tc_line)
    {
        if ($tc_line) {
            $length = count($tc_line);

            for ($i = 0; $i < $length; $i++) {
                $tc_transaction = new TcTransaction();
                $tc_transaction->tc_id      = $tc_line[$i];
                $tc_transaction->transaction_id     = $transaction->id;
                $tc_transaction->save();
            }
        }
    }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $dt = new DateTime();
        $output['current_datetime'] = $dt->format('d-m-Y h:i A');
        ...
        //Shop Contact Info
        $phone_contact_details = [];
        if ($il->show_mobile_number == 1 && !empty($location_details->mobile)) {
            $phone_contact_details[] = __('contact.mobile') . ': ' . $location_details->mobile;
        }
        if ($il->show_alternate_number == 1 && !empty($location_details->alternate_number)) {
            $phone_contact_details[] = $location_details->alternate_number;
        }

        $output['phone_contact'] = implode(', ', $phone_contact_details);
        $output['email'] = '';
        if ($il->show_email == 1 && !empty($location_details->email)) {
            $output['email'] = $location_details->email;
        }

        $output['contact'] = $output['phone_contact'];
        if ($output['email'] !== '') {
            $output['contact'] .= ($output['contact'] !== '' ? ', ' : '')
                . __('business.email') . ': ' . $output['email'];
        }
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_name'] = $customer->name;
            $output['business_name'] = $customer->supplier_business_name;
            $output['customer_mobile'] = $customer->mobile;
            $output['address_line_1'] = $customer->address_line_1;
            $output['customer_address'] = $customer->landline;

            if ($receipt_printer_type != 'printer') {
                // $output['contact_mobile'] = $customer->mobile;
                ...
                // $output['contact_to'] .= '<b>' . 'Contact To' . '</b>: ' .$customer->custom
        ...
        $output['shipping_status'] = $transaction->shipping_status;
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['invoice_status'] = 'quotation';
    ...
    public function getTermsCondition($transacion_id)
    {
        // $tc_transaction = TcTransaction::find($transacion_id);
        $tc_transaction = TcTransaction::where('transaction_id', $transacion_id)->get();

        // $tc_description = TermsCondition::whereIn('id', [1,2,3])->get();
        ...
        $tc_description = DB::table('terms_conditions')
            ...
            ->where('tc_transactions.transaction_id', 23)
            ->get()->toJson();
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftJoin('agent_transactions', 'transactions.invoice_no', '=', 'agent_transactions.invoice_no')
            ->join(
                'business_locations AS bl',
                'transactions.location_id',
                '=',
            ...
            ->select(
                ...
                'transactions.invoice_no',
                'agent_transactions.tracking_no',
                ...
                'transactions.custom_field_5',
                'transactions.custom_field_6',
